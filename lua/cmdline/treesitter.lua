--- Treesitter-based syntax highlighting for cmdline.nvim
--- Adapted from noice.nvim's text/treesitter.lua
local M = {}
M.queries = {}
...
--- Highlights a region of the buffer with a given language using treesitter
---@param buf integer? buffer to highlight
    ...
    for capture, node, metadata in iter do
      ...
      if hl and not is_spell then
        pcall(vim.api.nvim_buf_set_extmark, buf, ns, start_row, start_col, {
          ...
          end_col = end_col,
          hl_group = hl,
          priority = (tonumber(metadata.priority) or 100) + 10,
